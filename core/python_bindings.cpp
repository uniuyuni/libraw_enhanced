...
PYBIND11_MODULE(_core, m) {
  ...
  py::class_<ProcessingTimes>(m, "ProcessingTimes")
      ...
      .def_readonly("color_correction_time",
                    &ProcessingTimes::color_correction_time)
      .def_readonly("gamma_correction_time",
                    &ProcessingTimes::gamma_correction_time)
      ...
      .def_readonly("demosaic_algorithm_name",
                    &ProcessingTimes::demosaic_algorithm_name);
  ...
  py::class_<ProcessingParams>(m, "ProcessingParams")
      ...
      .def_readwrite("demosaic_algorithm",
                     &ProcessingParams::demosaic_algorithm)
      .def_readwrite("dcb_iterations", &ProcessingParams::dcb_iterations)
      .def_readwrite("dcb_enhance", &ProcessingParams::dcb_enhance)

      // Noise reduction parameters
      .def_readwrite("fbdd_noise_reduction",
                     &ProcessingParams::fbdd_noise_reduction)
      .def_readwrite("noise_thr", &ProcessingParams::noise_thr)
      .def_readwrite("median_filter_passes",
                     &ProcessingParams::median_filter_passes)
      ...
      .def_readwrite("output_color_space",
                     &ProcessingParams::output_color_space)
      ...
      .def_readwrite("adjust_maximum_thr",
                     &ProcessingParams::adjust_maximum_thr)
      ...
      .def_readwrite("exp_preserve_highlights",
                     &ProcessingParams::exp_preserve_highlights)
      ...
      .def_readwrite("chromatic_aberration_red",
                     &ProcessingParams::chromatic_aberration_red)
      .def_readwrite("chromatic_aberration_blue",
                     &ProcessingParams::chromatic_aberration_blue)
  ...
  py::class_<ImageBuffer>(m, "ImageBuffer")
      ...
      .def_property_readonly("image", [](ImageBuffer &self) -> py::object {
        if (self.image == nullptr) {
          return py::none();
        }
        // Return image data as array
        size_t total_size = self.height * self.width * self.channels;
        return py::array_t<uint16_t>(total_size,
                                     reinterpret_cast<uint16_t *>(self.image));
      });
  ...
  py::class_<ImageInfo>(m, "ImageInfo")
      ...
      .def_readonly("color_desc", &ImageInfo::color_desc)
      .def_readonly("is_xtrans", &ImageInfo::is_xtrans)
  ...
  py::class_<ImageBufferFloat>(m, "ImageBufferFloat")
      ...
      .def_property_readonly(
          "data", [](ImageBufferFloat &self) -> py::array_t<float> {
            if (!self.image || self.width == 0 || self.height == 0 ||
                self.channels == 0) {
  ...
  py::class_<ColorTransformMatrix>(m, "ColorTransformMatrix")
      ...
      .def_property_readonly(
          "transform",
          [](const ColorTransformMatrix &self) -> py::array_t<float> {
            return py::array_t<float>({3, 4},
  ...
  py::class_<ProcessedImageData>(m, "ProcessedImageData")
      ...
      .def(
          "to_numpy",
          [](ProcessedImageData &self) -> py::array {
            if (!self.is_valid()) {
              char errmsg[200];
              snprintf(errmsg, sizeof(errmsg),
                       "ProcessedImageData is invalid: width=%zu height=%zu "
                       "channels=%zu bps=%zu err=%d data=%p",
                       self.width, self.height, self.channels,
                       self.bits_per_sample, self.error_code, self.data);
            ...
            if (self.bits_per_sample == 8) {
              ...
              float *in_ptr = self.data;
              uint16_t *out_ptr =
                  static_cast<uint16_t *>(array_16.request().ptr);
              ...
              /*
                 ...
                 sizeof(float) * channels, sizeof(float)}, self.data
                              );
              */
            }
          },
      ...
      .def_property_readonly(
          "color_matrix",
          [](ProcessedImageData &self) -> py::array_t<float> {
            return py::array_t<float>({3, 4}, self.color_matrix.data(),
                                      py::cast(self));
          })
#endif
      ;
  ...
  py::class_<LibRawWrapper>(m, "LibRawWrapper")
      ...
      .def("get_raw_image_as_numpy",
           [](LibRawWrapper &self) { return self.get_raw_image_as_numpy(); })
  ...
  m.def("create_params_from_rawpy_args", &create_params_from_rawpy_args,
        ...
        // Basic parameters
        py::arg("use_camera_wb") = true, py::arg("half_size") = false,
        py::arg("four_color_rgb") = false, py::arg("output_bps") = 16,
        py::arg("user_flip") = -1,

        // Demosaicing parameters
        py::arg("demosaic_algorithm") = 1, py::arg("dcb_iterations") = 0,
        py::arg("dcb_enhance") = false,

        // Noise reduction parameters
        py::arg("fbdd_noise_reduction") = 0, py::arg("noise_thr") = 0.0f,
        ...
        // Brightness and exposure parameters
        py::arg("bright") = 1.0f, py::arg("no_auto_bright") = false,
        ...
        // Exposure correction parameters
        py::arg("exp_shift") = 1.0f, py::arg("exp_preserve_highlights") = 0.0f,

        // Gamma and scaling
        py::arg("gamma") =
            std::make_pair(0.f, 0.f), // std::make_pair(2.222f, 4.5f),
        ...
        // User adjustments
        py::arg("user_black") = -1, py::arg("user_sat") = -1,
  ...
  // Metal非対応環境用のスタブ関数
  m.def(
      "is_apple_silicon", []() { return false; },
      "Check if running on Apple Silicon (always false on non-Apple "
      "platforms)");
  m.def(
      "is_available", []() { return false; },
      "Check if available (always false on non-Apple platforms)");
  m.def(
      "get_device_list", []() { return std::vector<std::string>(); },
      "Get list of Metal devices (empty on non-Apple platforms)");
#endif
  ...
  // ユーティリティ関数
  m.def(
      "get_version_info", []() { return std::string(VERSION_INFO); },
