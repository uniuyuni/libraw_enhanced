...
struct ProcessingParams {
  ...
  float highlight_fringe_strength = 0.85f;  // 0.0-1.0 suppression strength
...
// Demosaicing algorithm enumeration - see constants.h for rawpy/LibRaw
...
private:
  std::unordered_map<void *, size_t>
      allocated_blocks_;     // {pointer, size_in_bytes}
...
public:
  ...
  bool demosaic_bayer_linear(const ImageBufferFloat &raw_buffer,
                             ImageBufferFloat &rgb_buffer, uint32_t filters,
                             float maximum_value);
  ...
  bool demosaic_bayer_amaze(const ImageBufferFloat &raw_buffer,
                            ImageBufferFloat &rgb_buffer, uint32_t filters,
                            const float (&cam_mul)[4], float maximum_value);

  // Unified demosaic compute method with CPU/GPU selection
  bool demosaic_compute(const ImageBufferFloat &raw_buffer,
                        ImageBufferFloat &rgb_buffer, int algorithm,
                        uint32_t filters, const char (&xtrans)[6][6],
  ...
  bool apply_white_balance(const ImageBuffer &raw_buffer,
                           ImageBufferFloat &rgb_buffer,
                           const float wb_multipliers[4], uint32_t filters,
                           const char xtrans[6][6]);
